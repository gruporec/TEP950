...
res.to_csv('resClasPCALDAMeteonD'+year_train+year_data+'.csv')
...
#si no existen, crea las carpetas 1_1 a 3_3 en figurasClasificadorPCALDAMeteonD
if not os.path.exists('figurasClasificadorPCALDAMeteonD'+year_train+year_data):
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data)
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/1_1')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/1_2')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/1_3')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/2_1')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/2_2')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/2_3')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/3_1')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/3_2')
    os.makedirs('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/3_3')

# abre una figura f
f=plt.figure(figsize=[len(res.loc['estado',:].values)/50+2,8],dpi=100)
# representa res.loc['estado',:]
plt.scatter(range(len(res.loc['estado',:].values)),res.loc['estado',:].values,color='red',label='Datos predichos',s=5)
# representa res.loc['valdata',:]
plt.scatter(range(len(res.loc['valdata',:].values)),res.loc['valdata',:].values,color='blue',label='Datos reales',s=1)
# agrega una leyenda
plt.legend()
...
f.savefig('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/ComparativaHD.png')
# cierra la figura f
plt.close(f)
sys.exit()
...
for i in range(len(res_estado)):
    ...
    for j in range(len(res_estado.columns)):
        ...
        # guarda la figura f en la carpeta figurasClasificadorConvolucion con el nombre de la columna de res_estado, indice de res_estado y valor de res_estado y valdatapd
        f.savefig('figurasClasificadorPCALDAMeteonD'+year_train+year_data+'/'+str(int(res_estado.iloc[i,j]))+'_'+str(int(res_valdata.iloc[i,j]))+'/'+res_estado.columns[j]+'_'+res_estado.index[i].strftime('%Y-%m-%d')+'.png')
        # cierra la figura f
        plt.close(f)
