...
pca = skdecomp.PCA(n_components=4)
pca.fit(Xtr)
...
cols2=pltcolor(valdatapd.unstack().values)
cols=pltcolor(trdatapd.unstack().values)

#crea una figura con 4 subplots
fig, axs = plt.subplots(2, 2, figsize=(10,10))
# representa los datos de entrenamiento
axs[0,0].scatter(Xtr_pca[:,0],Xtr_pca[:,1],c=cols,s=1)
axs[0,1].scatter(Xtr_pca[:,0],Xtr_pca[:,2],c=cols,s=1)
axs[1,0].scatter(Xtr_pca[:,0],Xtr_pca[:,3],c=cols,s=1)
axs[1,1].scatter(Xtr_pca[:,1],Xtr_pca[:,2],c=cols,s=1)
# crea otra figura con 4 subplots
fig, axs = plt.subplots(2, 2, figsize=(10,10))
# representa los datos de validación
axs[0,0].scatter(Xv_pca[:,0],Xv_pca[:,1],c=cols2,s=1)
axs[0,1].scatter(Xv_pca[:,0],Xv_pca[:,2],c=cols2,s=1)
axs[1,0].scatter(Xv_pca[:,0],Xv_pca[:,3],c=cols2,s=1)
axs[1,1].scatter(Xv_pca[:,1],Xv_pca[:,2],c=cols2,s=1)
...
res.to_csv('resClasPCALDA'+year_train+year_data+'.csv')
...
#si no existen, crea las carpetas 1_1 a 3_3 en figurasClasificadorPCALDA
if not os.path.exists('figurasClasificadorPCALDA'+year_train+year_data):
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data)
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/1_1')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/1_2')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/1_3')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/2_1')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/2_2')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/2_3')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/3_1')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/3_2')
    os.makedirs('figurasClasificadorPCALDA'+year_train+year_data+'/3_3')
...
for i in range(len(res_estado)):
    ...
    for j in range(len(res_estado.columns)):
        ...
        f.savefig('figurasClasificadorPCALDA'+year_train+year_data+'/'+str(int(res_estado.iloc[i,j]))+'_'+str(int(res_valdata.iloc[i,j]))+'/'+res_estado.columns[j]+'_'+res_estado.index[i].strftime('%Y-%m-%d')+'.png')
        # cierra la figura f
        plt.close(f)
