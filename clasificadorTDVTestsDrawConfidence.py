...
def process_el(args):
    ...
    try:
        ...
        # Traza una línea en 1-x-y=x; 1-2x-y=0; y=1-2x
        x=np.linspace(0.33,1,100)
        # plt.plot(x, 1-2*x, color='black')

        # # Traza una línea en 1-x-y=y; 1-x-2y=0; y=1/2-x/2
        # plt.plot(np.linspace(0,0.33,100), (1-np.linspace(0,0.33,100))/2, color='black')

        # # Traza una linea entre 0.33,0.33 y 1,1
        # plt.plot([0.33,1],[0.33,1], color='black')
        ...
        x_data = np.cos(np.pi/6)*(data_test_pred_proba[2]-data_test_pred_proba[0])
        y_data = data_test_pred_proba[1]-0.5*(data_test_pred_proba[2]+data_test_pred_proba[0])
        ...
        # # Pinta en azul a 50% de transparencia el área por encima de 1-2y=x a la izquierda de x=0.33
        # plt.fill_between(np.linspace(0,0.33,100), (1-np.linspace(0,0.33,100))/2, 1 , color='blue', alpha=alpha)
        # # Pinta en azul a 50% de transparencia el área por encima de x=y a la derecha de x=0.33
        # plt.fill_between(np.linspace(0.33,1,100), np.linspace(0.33,1,100), 1 , color='blue', alpha=alpha)
        # # Pinta en verde a 50% de transparencia el área por debajo de 1-2x=y a la izquierda de x=0.33
        # plt.fill_between(np.linspace(0,0.33,100), 0 , (1-np.linspace(0,0.33,100))/2 , color='green', alpha=alpha)
        # # Pinta en verde a 50% de transparencia el área por debajo de 1-2y=x a la derecha de x=0.33
        # plt.fill_between(x, 0, 1-2*x, color='green', alpha=alpha)
        ...
        plt.scatter(x_data, y_data, c=data_test_pred_proba['real'].apply(lambda x: colors[int(x-1)]), marker=',', s=0.1)
        #plt.scatter(data_test_pred_proba[0], data_test_pred_proba[2], c=data_test_pred_proba['real'].apply(lambda x: colors[int(x-1)]), marker=',', s=1)
        #recorta los ejes a 0,1
        # plt.xlim(0,1)
        # plt.ylim(0,1)
        

        # Añade etiquetas a los ejes y título
        # plt.xlabel('Probabilidad de estar en un nivel de estrés 1')
        # plt.ylabel('Probabilidad de estar en un nivel de estrés 3')
        plt.title(year_test)
        ...
        # Guarda la imagen
        plt.savefig(save_folder+'lda_' + str(year_test) + '_small_marker.png', dpi=300)
        #plt.show()
