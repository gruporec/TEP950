...
# normaliza las secuencias restando la media y dividiendo por la desviación estándar
ltp_1=(ltp_1-ltp_1.mean())/ltp_1.std()
ltp_2=(ltp_2-ltp_2.mean())/ltp_2.std()
ltp_3=(ltp_3-ltp_3.mean())/ltp_3.std()
...
for i in range(Xv.shape[0]):
    # normaliza la fila de Xv restando la media y dividiendo por la desviación estándar
    Xv_n=(Xv[i,:]-Xv[i,:].mean())/Xv[i,:].std()
    # calcula la convolución de Xv[:,i] con ltp_1
    conv_1=np.convolve(Xv_n,ltp_1_flip)
    # calcula la convolución de Xv[:,i] con ltp_2
    conv_2=np.convolve(Xv_n,ltp_2_flip)
    # calcula la convolución de Xv[:,i] con ltp_3
    conv_3=np.convolve(Xv_n,ltp_3_flip)
    ...
    max_pos[0,i]=max_1_pos
    max_pos[1,i]=max_2_pos
    max_pos[2,i]=max_3_pos

    # clasifica la curva en función de la máxima de las convoluciones y almacena la posición del máximo correspondiente
...
res.to_csv('resClasConv'+year_train+year_data+'.csv')
...
#si no existen, crea las carpetas 1_1 a 3_3 en figurasClasificadorConv2
if not os.path.exists('figurasClasificadorConv'+year_train+year_data+''):
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/1_1')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/1_2')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/1_3')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/2_1')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/2_2')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/2_3')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/3_1')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/3_2')
    os.makedirs('figurasClasificadorConv'+year_train+year_data+'/3_3')
...
for i in range(len(res_estado)):
    ...
    for j in range(len(res_estado.columns)):
        ...
        t_1 = t - res_max_pos1.iloc[i,j]+len(t)
        t_2 = t - res_max_pos2.iloc[i,j]+len(t)
        t_3 = t - res_max_pos3.iloc[i,j]+len(t)
        ...
        f.savefig('figurasClasificadorConv'+year_train+year_data+'/'+str(int(res_estado.iloc[i,j]))+'_'+str(int(res_valdata.iloc[i,j]))+'/'+res_estado.columns[j]+'_'+res_estado.index[i].strftime('%Y-%m-%d')+'.png')
        # cierra la figura f
        plt.close(f)