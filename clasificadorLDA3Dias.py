...
# crea ltp_2 copiando ltp_1
ltpP_2 = ltpP_1.copy()
ltpT_2 = ltpT_1.copy()

# resta 24 a Hora_norm en ltp_2
ltpP_2.loc[:,'Hora_norm'] = ltpP_2.loc[:,'Hora_norm'] - 24
ltpT_2.loc[:,'Hora_norm'] = ltpT_2.loc[:,'Hora_norm'] - 24

# suma 1 dia al índice de ltp_2
ltpP_2.index = ltpP_2.index + pd.Timedelta(days=1)
ltpT_2.index = ltpT_2.index + pd.Timedelta(days=1)

# combina ltp, ltp_1 y ltp_2
ltpP = pd.concat([ltpP,ltpP_1,ltpP_2])
ltpT = pd.concat([ltpT,ltpT_1,ltpT_2])
...
res.to_csv('resClasLDA3Dias'+year_train+year_data+'.csv')
...
# extiende las secuencias 3 veces
ltp_1 = np.tile(ltp_1,3)
ltp_2 = np.tile(ltp_2,3)
ltp_3 = np.tile(ltp_3,3)

#si no existen, crea las carpetas 1_1 a 3_3 en figurasClasificadorLDA3Dias
if not os.path.exists('figurasClasificadorLDA3Dias'+year_train+year_data):
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data)
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/1_1')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/1_2')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/1_3')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/2_1')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/2_2')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/2_3')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/3_1')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/3_2')
    os.makedirs('figurasClasificadorLDA3Dias'+year_train+year_data+'/3_3')
...
for i in range(len(res_estado)):
    ...
    for j in range(len(res_estado.columns)):
        ...
        f.savefig('figurasClasificadorLDA3Dias'+year_train+year_data+'/'+str(int(res_estado.iloc[i,j]))+'_'+str(int(res_valdata.iloc[i,j]))+'/'+res_estado.columns[j]+'_'+res_estado.index[i].strftime('%Y-%m-%d')+'.png')
        # cierra la figura f
        plt.close(f)
