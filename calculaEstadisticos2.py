...
# obtiene los valores de la pendiente de ltp en los cambios de signo a negativo
ltp_diff_R_Neta_Avg = ltp_diff.loc[signos_pn.index]
...
ltp_R_Neta_Avg = ltp.loc[signos_pn.index]
ltp_R_Neta_Avg = ltp_R_Neta_Avg.resample('D').last()
# obtiene el valor de ltp una hora después
ltp_R_Neta_Avg_1h = ltp.loc[signos_pn.index+pd.Timedelta(hours=1)]
...
ltpn_diff_R_Neta_Avg = ltpn_diff.loc[signos_pn.index]
# remuestrea al último valor diario
...
ltpn_R_Neta_Avg = ltpn.loc[signos_pn.index]
ltpn_R_Neta_Avg = ltpn_R_Neta_Avg.resample('D').last()
ltpn_R_Neta_Avg_1h = ltpn.loc[signos_pn.index+pd.Timedelta(hours=1)]
ltpn_R_Neta_Avg_1h = ltpn_R_Neta_Avg_1h.resample('D').last()
ltpn_diff_1h = ltpn_R_Neta_Avg_1h - ltpn_R_Neta_Avg

# obtiene el valor en el primer cambio de signo negativo a positivo
ltpn_R_Neta_Avg_1h_1 = ltpn.loc[signos_np.index+pd.Timedelta(hours=1)]
