...
def process_el(args):
    (TreeCount,TDV_days,years_test,year_train,chars,tdv_train_orig,data_train_orig,tdv_tests_orig,data_tests_orig)=args
    ...
    try:
        ...
        for yt in range(len(years_test)):
            ...
            #crea un modelo random forest de sklearn
            rf = skens.RandomForestClassifier(n_estimators=TreeCount)

            #entrena el modelo con los datos de tdv_prev
            rf.fit(tdv_meteo_train,data_train)

            #aplica el clasificador a los datos de tdv_meteo
            data_train_pred = rf.predict(tdv_meteo_train)
            data_test_pred = rf.predict(tdv_meteo_test)
        ...
        return (avg_accuracy,TreeCount,TDV_days,years_test,year_train)
        ...
        print('TreeCount: ' + str(TreeCount),'TDV_days: ' + str(TDV_days))
        sys.exit()
        return 0
...
if __name__=='__main__':
    ...
    meta_file='ignore/resultadosTDV/batch/RForest/meta2.csv'
    ...
    tests = pd.read_csv('ignore/resultadosTDV/batch/RForest/programmedTests3.csv')
    ...
    for index, row in tests.iterrows():
        if row['done']==False:
            ...
            res_file_target='ignore/resultadosTDV/batch/RForest/'+str(index)
            ...
            if os.path.isfile(res_file):
                ...
                file.write("TreeCount,TDV_days,test_score\n")
            ...
            tests.to_csv('ignore/resultadosTDV/batch/RForest/programmedTests3.csv',index=True)
