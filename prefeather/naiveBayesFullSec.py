...
traindatapd=pd.read_csv("calibracion3.csv")
traindatapd.dropna(inplace=True)
traindatapd['Fecha'] = pd.to_datetime(traindatapd['Fecha'])
traindatapd.set_index('Fecha',inplace=True)
...
traindatapd = traindatapd.resample('T').pad()

# elimina los valores de ltp que no estén en valdatapd
ltpv = ltp.loc[traindatapd.index,traindatapd.columns]
# elimina los valores de diff que no estén en valdatapd
ltp_diffv = ltp_diff.loc[traindatapd.index,traindatapd.columns]
# elimina los valores de diff2 que no estén en valdatapd
ltp_diff2v = ltp_diff2.loc[traindatapd.index,traindatapd.columns]
