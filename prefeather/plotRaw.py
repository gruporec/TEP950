...
for column in ltpPlot:
    for idx, day in ltpPlot[column].groupby(ltpPlot[column].index.date):
        currFolder=saveFolder+'/'+column
        if not os.path.exists(currFolder):
            os.makedirs(currFolder)

        meteod=meteoPlot[idx:idx+pd.Timedelta(n_dias_print,'d')]
        ltpd=ltpPlot[column][idx:idx+pd.Timedelta(n_dias_print,'d')]

        fig, [ax1,ax2,ax3,ax4,ax5] = plt.subplots(5,1)

        ax1.plot(ltpd)
        ax1.set(xlabel='Hora', ylabel='LTP', title=column+' '+str(idx+pd.Timedelta(n_dias_print-1,'d')))
