...
class KriggingClassifier:
    ...
    def update_matrices(self, Xtrain, alpha, beta):
        ...
        self.P = np.zeros([2*Xtrain.shape[0]+1, 2*Xtrain.shape[0]+1])
        self.P[:Xtrain.shape[0], :Xtrain.shape[0]] = np.eye(Xtrain.shape[0])*2
        # Convierte P en una matriz dispersa
        self.P = sp.csc_matrix(P)
        ...
        self.q = np.zeros([2*Xtrain.shape[0]+1])
        self.q[:Xtrain.shape[0]] = beta
        self.q[Xtrain.shape[0]:2*Xtrain.shape[0]] = alpha
        self.q[2*Xtrain.shape[0]] = beta
        ...
        self.G = np.zeros([2*Xtrain.shape[0], 2*Xtrain.shape[0]+1])
        self.G[:Xtrain.shape[0], :Xtrain.shape[0]] = -np.eye(Xtrain.shape[0])
        self.G[Xtrain.shape[0]:2*Xtrain.shape[0], Xtrain.shape[0]:2*Xtrain.shape[0]] = -np.eye(Xtrain.shape[0])
        self.G[Xtrain.shape[0]:2*Xtrain.shape[0], :Xtrain.shape[0]] = -np.eye(Xtrain.shape[0])
        self.G[:Xtrain.shape[0], Xtrain.shape[0]:2*Xtrain.shape[0]] = np.eye(Xtrain.shape[0])

        # Calcula el vector h de tamaño 2*N con valores cero
        self.h = np.zeros([2*Xtrain.shape[0]])
        ...
        self.A[:Xtrain.shape[0], :Xtrain.shape[0]] = Xtrain
        self.A[Xtrain.shape[0], 2*Xtrain.shape[0]] = 1
    ...
    def apply(self,x):
        ...
        T = qp.solve_qp(self.P, self.q, self.G, self.h, self.A, b, solver='osqp')

        # Obtiene el valor de la función objetivo
        f = 0.5*np.dot(T, np.dot(self.P, T)) + np.dot(self.q, T)

        # Obtiene el vector de lambdas de los datos de entrenamiento
        lambda_i = T[:self.Xtrain.shape[0]]
...
def Krigging(inputs):
    (x_i, X_train, y_train, classes, num_classes, P, q, G, h, A, P_class, q_class, G_class, h_class, A_class) = inputs
    ...
    scores_test_class = np.zeros([num_classes])
    # Para cada clase
    ...
    #calcula el T que minimiza la función Qp usando OSQP para cada A_class
    for c in range(len(classes)):
        #calcula el T que minimiza la función Qp usando OSQP
        T_c = qp.solve_qp(P_class[c], q_class[c], G_class[c], h_class[c], A_class[c], b, solver='osqp')
        #comprueba si T_c es none
        if T_c is None:
            #si es none, asigna un valor alto
            scores_test_class[c] = 1e10
        else:
            scores_test_class[c] = 1/2*np.dot(T_c.T,P_class[c].dot(T_c))+np.dot(q_class[c].T,T_c)


    #revisa que A*T=b con una tolerancia de 1e-4 para el error máximo
    # if np.max(np.abs(A.dot(T)-b))>1e-4:
    #revisa que A*T=b con una tolerancia del 10% para el error máximo
    # if np.max(np.divide(np.abs(A.dot(T)-b),b))>0.05:
    #     print(A.dot(T)-b)
    # else:
    #     print("OK")
    #print(np.max(np.divide(np.abs(A.dot(T)-b),b)))

    # devuelve la clase con mayor score para el dato de prueba
    #return classes[np.argmax(scores_test[:])]
    return classes[np.argmin(scores_test_class)]
...
if __name__ == "__main__":
    ...
    for year_data in year_datas:
        ...
        P = np.zeros([2*Xtr.shape[0]+1, 2*Xtr.shape[0]+1])
        P[:Xtr.shape[0], :Xtr.shape[0]] = np.eye(Xtr.shape[0])*2
        # Convierte P en una matriz dispersa
        P = sp.csc_matrix(P)
        ...
        G = np.zeros([2*Xtr.shape[0], 2*Xtr.shape[0]])
        G[:Xtr.shape[0], :Xtr.shape[0]] = -np.eye(Xtr.shape[0])
        G[Xtr.shape[0]:, Xtr.shape[0]:] = -np.eye(Xtr.shape[0])
        G[:Xtr.shape[0], Xtr.shape[0]:] = np.eye(Xtr.shape[0])
        G[Xtr.shape[0]:, :Xtr.shape[0]] = -np.eye(Xtr.shape[0])
        ...
        # A = np.zeros([Xtr.shape[1]+1, 2*Xtr.shape[0]])
        # A[:Xtr.shape[1], :Xtr.shape[0]] = Xtr.T
        # A[Xtr.shape[1], :Xtr.shape[0]] = 1
        A = np.zeros([Xtr.shape[1], 2*Xtr.shape[0]])
        A[:Xtr.shape[1], :Xtr.shape[0]] = Xtr.T
        # Convierte A en una matriz dispersa
        A = sp.csc_matrix(A)

        #crea una tupla con los datos de entrenamiento de cada clase
        Xtr_class = tuple([Xtr[Ytr==c] for c in classes])

        #crea una lista con la matriz P de cada clase
        P_class = ["" for c in range(len(classes))]
        #crea una lista con la matriz q de cada clase
        q_class = ["" for c in range(len(classes))]
        #crea una lista con la matriz G de cada clase
        G_class = ["" for c in range(len(classes))]
        #crea una lista con la matriz h de cada clase
        h_class = ["" for c in range(len(classes))]
        #crea una lista con la matriz A de cada clase
        A_class = ["" for c in range(len(classes))]
        #calcula las matrices P, q, G, h y A de cada clase
        for c in range(len(classes)):
            # calcula la matriz P de cada clase
            P_c = np.zeros([2*Xtr_class[c].shape[0], 2*Xtr_class[c].shape[0]])
            P_c[:Xtr_class[c].shape[0], :Xtr_class[c].shape[0]] = np.eye(Xtr_class[c].shape[0])*2
            # Convierte P en una matriz dispersa
            P_c = sp.csc_matrix(P_c)
            P_class[c] = P_c

            # calcula la matriz q de cada clase
            q_c = np.zeros([2*Xtr_class[c].shape[0], 1])
            q_c[Xtr_class[c].shape[0]:] = alph
            q_class[c] = q_c

            # calcula la matriz G de cada clase
            G_c = np.zeros([2*Xtr_class[c].shape[0], 2*Xtr_class[c].shape[0]])
            G_c[:Xtr_class[c].shape[0], :Xtr_class[c].shape[0]] = -np.eye(Xtr_class[c].shape[0])
            G_c[Xtr_class[c].shape[0]:, Xtr_class[c].shape[0]:] = -np.eye(Xtr_class[c].shape[0])
            G_c[:Xtr_class[c].shape[0], Xtr_class[c].shape[0]:] = np.eye(Xtr_class[c].shape[0])
            G_c[Xtr_class[c].shape[0]:, :Xtr_class[c].shape[0]] = -np.eye(Xtr_class[c].shape[0])
            # Convierte G en una matriz dispersa
            G_c = sp.csc_matrix(G_c)
            G_class[c] = G_c

            # calcula la matriz h de cada clase
            h_c = np.zeros([2*Xtr_class[c].shape[0], 1])
            h_class[c] = h_c

            #calcula la matriz A de cada clase
            A_c = np.zeros([Xtr_class[c].shape[1]+1, 2*Xtr_class[c].shape[0]])
            A_c[:Xtr_class[c].shape[1], :Xtr_class[c].shape[0]] = Xtr_class[c].T
            A_c[Xtr_class[c].shape[1], :Xtr_class[c].shape[0]] = 1
            # Convierte A en una matriz dispersa
        ...
        inputs = [(Xv[i], Xtr, Ytr, classes, num_classes, P, q, G, h, A, P_class, q_class, G_class, h_class, A_class) for i in range(Xv.shape[0])]
        ...
        y_test_pred = list(tqdm.tqdm(pool.imap(Krigging, inputs), total=Xtr.shape[0],disable=True))
