...
max = tdv.resample('1D').max()
max['Carac'] = 'max'

max['Fecha'] = max.index.date

#obtiene el mínimo diario de tdv
min = tdv.resample('1D').min()
min['Carac'] = 'min'

min['Fecha'] = min.index.date

#obtiene el mínimo del día anterior de tdv
min_ant = tdv.resample('1D').min()
min_ant.index = min_ant.index + pd.Timedelta(days=1)
min_ant['Fecha'] = min_ant.index.date


#obtiene la diferencia entre el máximo y el mínimo del día anterior de tdv
dif = max - min_ant
dif['Carac'] = 'dif'
#resetea la columna de fecha al índice
dif['Fecha'] = dif.index.date
print(dif)

# #remuestrea tdv a 1 hora
# tdv = tdv.resample('1H').mean()

#crea un dataframe para guardar las características de tdv
tdv_char=max.copy()
#añade la columna de mínimos al dataframe de características
tdv_char = tdv_char.append(min)
#añade la columna de diferencias al dataframe de características
tdv_char = tdv_char.append(dif)
...
for i in range(1,nDias,1):
    tdv_temp = tdv_char.copy()
    #añade i a la columna Carac
    tdv_temp['Carac'] = tdv_temp['Carac'] + ' -' + str(i) + 'd'
