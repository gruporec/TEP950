...
#tdv=tdv.diff(periods=60*24)#+tdv.diff(periods=60*24*2)#*tdv.diff(periods=60*24)*tdv.diff(periods=60*24)#*np.sign(tdv.diff(periods=60*24))
...
tdv_diff = tdv.diff(periods=60*24)
...
tdv_diff2 = tdv.diff(periods=2*60*24)
...
print([tdv_1.max().max(),tdv_2.max().max(),tdv_3.max().max()])
ymax=max([tdv_1.max().max(),tdv_2.max().max(),tdv_3.max().max()])
ymin=min([tdv_1.min().min(),tdv_2.min().min(),tdv_3.min().min()])
ymax2=max([diff_1.max().max(),diff_2.max().max(),diff_3.max().max()])
ymin2=min([diff_1.min().min(),diff_2.min().min(),diff_3.min().min()])
ymax3=max([diff2_1.max().max(),diff2_2.max().max(),diff2_3.max().max()])
ymin3=min([diff2_1.min().min(),diff2_2.min().min(),diff2_3.min().min()])
# graficas en negro
...
fig.savefig('figuras/figuras dias/TDV2f24hdiarios'+year+'.png')
fig2.savefig('figuras/figuras dias/TDV2f24hmedias'+year+'.png')
