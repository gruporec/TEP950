...
df = pd.read_csv("ignore/analisisPCALDA/resultadosPCALDAnoMeteo2.csv", index_col=[0,1,2,3],header=[0,1], skipinitialspace=True)
print(df)
accuracies=df['total accuracy'].droplevel(3,'index').unstack(level=0).droplevel(0,'columns').swaplevel(0,1,'index').sort_index('index',0)
print(accuracies)
