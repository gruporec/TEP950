...
def process_el(args):
    ...
    try:
        ...
        for yt in range(len(years_test)):
            ...
            #crea un clasificador nearest neighbors de sklearn
            skneighbors = skneigh.KNeighborsClassifier(n_neighbors=5,weights='distance')

            #entrena el clasificador con los datos de tdv_meteo
            skneighbors.fit(tdv_meteo_train_f,data_train)

            #aplica el clasificador a los datos de tdv_meteo
            data_train_pred = skneighbors.predict(tdv_meteo_train_f)
            data_test_pred = skneighbors.predict(tdv_meteo_test_f)
...
if __name__=='__main__':
    ...
    meta_file='ignore/resultadosTDV/batch/PCAKNN/meta2.csv'
    ...
    tests = pd.read_csv('ignore/resultadosTDV/batch/PCAKNN/programmedTests3.csv')
    ...
    for index, row in tests.iterrows():
        if row['done']==False:
            ...
            res_file_target='ignore/resultadosTDV/batch/PCAKNN/'+str(index)
            ...
            for epoch in range(epochs):
                ...
                for batch_el in range(batch_size):
                    ...
                    PCA_components=search_PCA_components+np.random.randint(max(-temperature*pca_temp_scale,1-search_PCA_components),min(temperature*pca_temp_scale,(1+num_params*TDV_days)-search_PCA_components))
            ...
            tests.to_csv('ignore/resultadosTDV/batch/PCAKNN/programmedTests3.csv',index=True)
