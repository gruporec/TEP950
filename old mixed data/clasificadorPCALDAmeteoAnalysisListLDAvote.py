        ...
        for ltpitems in lptlist:
            for meteoitems in meteolist:
                ...
                for comp in LDAlistcrop: #range(1,XtrBase.shape[1]+1,math.ceil((XtrBase.shape[1]+1)/10)):
                    ...
                    if 'Yprobsum' not in locals():
                        Yprobsum=Yprob
                    else:
                        Yprobsum=np.add(Yprob,Yprobsum)
                    YpredVote=Yprobsum.argmax(axis=1)+1
                    ...
                    print(bcm)
                    bcm=skmetrics.confusion_matrix(Yv, YpredVote,normalize='true')
                    print(bcm)
    
                    # calcula el porcentaje de acierto
                    accuracy = skmetrics.balanced_accuracy_score(Yv, YpredVote)
