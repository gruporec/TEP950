...
def process_el(args):
    ...
    try:
        ...
        if norm:
            ...
            tdv_train_m = tdv_train.between_time('5:00', '8:00')
            tdv_test_m = tdv_test.between_time('5:00', '8:00')
            
            #obtiene el máximo diario de tdv
            tdv_train_max = tdv_train_m.groupby(tdv_train_m.index.date).max()
            tdv_test_max = tdv_test_m.groupby(tdv_test_m.index.date).max()
        ...
        # Añade también el valor predicho
        data_test_pred_proba['pred'] = data_test_pred

        # Añade como índice el índice de data_test
        data_test_pred_proba.index = data_test.index

        # Guarda el resultado en un fichero en save_folder
        data_test_pred_proba.to_csv(save_folder + str(year_test) +'_data_test_pred_proba.csv')
...
if __name__=='__main__':
    ...
    row=(True,True,True,False,False,False,False) 
    ...
    save_folder = 'ignore/resultadosTDV/batch/PCALDA6am/IMG/AnalisisID80/'
