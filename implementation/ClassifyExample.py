...
origdata = origdata.loc[origdata['year'].isin(years_test)]
...
# plot the processed data (one sample)
fig, ax = plt.subplots()
ax.plot(processed_data.iloc[0])
plt.show()

# load the classifier
classifier = f.loadModel(classifierFolder+'/'+classifierFile)

# predict the stress level
predicted_stress = classifier.predict(processed_data)

# calculate the confusion matrix
cm = confusion_matrix(stress, predicted_stress)
