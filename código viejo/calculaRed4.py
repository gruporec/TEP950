...
def calculaEstSum(dat,design_variables,minexp,dia):#a,an,m,r,mn

    est=calculaEst(dat,design_variables,minexp)
...
def calculaEstim(dat,design_variables,minexp):#,fini,fend):#a,an,m,pre,mn,mx=10
    lpti=dat[0].astype(float)
    lptmax=dat[1].astype(float)
    eTo=dat[2].astype(float)
    estInd = pd.DataFrame(0,index=lpti.index,columns=lpti.columns)
    for x in range(len(design_variables)):
        for y in range(len(design_variables[x])):
            for z in range(len(design_variables[x][y])):
                for w in range(len(design_variables[x][y][z])):
                    #print(design_variables[x][y][z][w])
                    estInd.iloc[0]+=design_variables[x][y][z][w]*lpti.iloc[0].pow(x+minexp)*lptmax.iloc[0].pow(y+minexp)*eTo.iloc[0].pow(z+minexp)
    ...
    for i in range(1, len(estInd)):
        for x in range(len(design_variables)):
            for y in range(len(design_variables[x])):
                for z in range(len(design_variables[x][y])):
                    for w in range(len(design_variables[x][y][z])):
                        #print("calculating minterm")
                        estInd.iloc[i]+=design_variables[x][y][z][w]*lpti.iloc[i].pow(x+minexp)*lptmax.iloc[i].pow(y+minexp)*eTo.iloc[i].pow(z+minexp)*estInd.iloc[i-1].pow(w+minexp)

        
    return estInd

def calculaEst(dat,design_variables,minexp):
    estInd=calculaEstim(dat,design_variables,minexp)
    ...
    for x in range(len(x0)):
        for y in range(len(x0[x])):
            for z in range(len(x0[x][y])):
                for w in range(len(x0[x][y][z])):
                    for a in range(-1,2,2): 
                        x0=cp.deepcopy(x1)
                        x0[x][y][z][w]+=a*step
                        est=calculaEst(dat,x0,minexp)
...
estim=calculaEstim(dat,x0,minexp)
...
df.to_csv("estadisticosDiariosConEst4.csv", index=True)
