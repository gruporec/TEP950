...
df2 = pd.read_csv(folder+year+" hoja 2.csv",na_values='.') # Estados diarios
...
# Guarda datos RAW
dfd.to_csv("rawDiarios"+year+".csv", index=False)

df31 = pd.read_csv(folder+year+" hoja 31.csv",na_values='.') # TDV freq variable
...
df32 = pd.read_csv(folder+year+" hoja 32.csv",na_values='.') # LTP cada 5 minutos con desviación de 1 minuto
...
df4 = pd.read_csv(folder+year+" hoja 4.csv",na_values='.') # Meteo cada 30 minutos
...
for col in range (1,len(df4.columns)):
    df4.iloc[:,col]=pd.to_numeric(df4.iloc[:,col], errors='coerce')
...
dfT.to_csv("rawMinutales"+year+".csv", index=True)