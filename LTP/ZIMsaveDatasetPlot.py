...
for year_data in year_datas:
    ...
    # order meteoP columns
    meteoP = meteoP.reindex(sorted(meteoP.columns), axis=1)

    # rename columns of meteoP from ['H_Relat_Avg', 'R_Neta_Avg', 'T_Amb_Avg', 'VPD_Avg'] to ['H', 'Rad', 'T', 'VPD']
    meteoP.columns = ['H', 'Rad', 'T', 'VPD']

    # select only the sensorPlot column from ltpP
    ltpP=ltpP.loc[:,ltpP.columns.str.startswith(sensorPlot)]

    # select only the sensorPlot column from valdatapd
    valdatapd=valdatapd.loc[:,valdatapd.columns.str.startswith(sensorPlot)]

    # rename columns of ltpP from ['LTP Control_2_1'] to ['LTP']
    ltpP.columns = ['LTP']

    # rename columns of valdatapd from ['LTP Control_2_1'] to ['LTP']
    valdatapd.columns = ['LTP']

    # add meteo to ltpP
    ltpP=ltpP.join(meteoP)

    # extract the dayPlot from ltpP
    plotData=ltpP.loc[dayPlot,:]

    # plot ltpP
    plotData.plot(subplots=True,title="Raw data")
    ...
    # plot ltpP
    plotData.plot(subplots=True,title="Filtered data")
    ...
    # plot ltpP
    plotData.plot(subplots=True,title="Normalized data")

    # get every sign change of R_Neta_Avg in meteo
    signosP = np.sign(meteoP.loc[:,meteoP.columns.str.startswith('Rad')]).diff()
    ...
    #drop the Hora_norm column from plotData
    plotData = plotData.drop('Hora_norm',axis=1)

    # plot ltpP
    plotData.plot(subplots=True,title="Cropped data")

    # create a second figure with the same plot
    plotData.plot(subplots=True,title="Resampling data")

    # get the number of subplots
    nsubplots = len(plotData.columns)

    #modify the first subplot to add sampling divisions
    ax = plt.subplot(nsubplots,1,1)
    # get the x axis limits
    xlim = ax.get_xlim()
    # divide the x axis in ltpitems parts
    xticks = np.linspace(6,18,ltpitems+1)
    # draw vertical lines in the x axis
    for xtick in xticks:
        ax.axvline(x=xtick,linewidth=0.5,color='k')
    
    #divide the x axis in meteoitems parts
    if meteoitems>0:
        xticks = np.linspace(6,18,meteoitems+1)
        # draw vertical lines in the x axis in all other subplots
    ...
    if meteoitems>0:
        ...
        # order meteoP_norm by its second level index
        meteoP_norm = meteoP_norm.sort_index(level=1)
...
# drop the first level of the index of savedfX
savedfX.index = savedfX.index.droplevel(0)

# extract the dayPlot from savedfX
plotData=savedfX.loc[dayPlot,:]

# plot the ltp part of the data
plt.figure()
plt.stem(plotData.index[0:ltpitems],plotData.values[0:ltpitems])

# get the number of meteo parts
nmeteo = plotData.shape[0]-ltpitems
nmeteo = int(nmeteo/meteoitems)

# plot the meteo parts of the data
for i in range(nmeteo):
    plt.stem(plotData.index[ltpitems+i*meteoitems:ltpitems+(i+1)*meteoitems],plotData.values[ltpitems+i*meteoitems:ltpitems+(i+1)*meteoitems], linefmt ='C'+str(i+1)+'-')


plt.title("Data vector")
