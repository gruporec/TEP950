...
# calcula las características de las curvas
# calcula los valores máximos de cada columna de meteo_norm y ltp
ltpvmax=ltpv.max(axis=('rows'))
ltpvmaxloc=ltpv.idxmax(axis=('rows'))

ltptmax=ltpt.max(axis=('rows'))
ltptmaxloc=ltpt.idxmax(axis=('rows'))

meteoPmax=meteoP_norm.max(axis=('rows'))
meteoPmaxloc=meteoP_norm.idxmax(axis=('rows'))

meteoTmax=meteoT_norm.max(axis=('rows'))
meteoTmaxloc=meteoT_norm.idxmax(axis=('rows'))

# calcula los valores mínimos de cada columna de meteo_norm y ltp
ltpvmin=ltpv.min(axis=('rows'))
ltpvminloc=ltpv.idxmin(axis=('rows'))

ltptmin=ltpt.min(axis=('rows'))
ltptminloc=ltpt.idxmin(axis=('rows'))

meteoPmin=meteoP_norm.min(axis=('rows'))
meteoPminloc=meteoP_norm.idxmin(axis=('rows'))

meteoTmin=meteoT_norm.min(axis=('rows'))
meteoTminloc=meteoT_norm.idxmin(axis=('rows'))

# calcula los valores medios de cada columna de meteo_norm y ltp
ltpvavg=ltpv.mean(axis=('rows'))

ltptavg=ltpt.mean(axis=('rows'))

meteoPavg=meteoP_norm.mean(axis=('rows'))

meteoTavg=meteoT_norm.mean(axis=('rows'))

# calcula la varianza de cada columna de meteo_norm y ltp
ltpvvar=ltpv.var(axis=('rows'))

ltptvar=ltpt.var(axis=('rows'))

meteoPvar=meteoP_norm.var(axis=('rows'))

meteoTvar=meteoT_norm.var(axis=('rows'))

# calcula la pendiente entre el punto máximo y el punto mínimo de cada columna de meteo_norm y ltp
ltpvslope=(ltpvmax-ltpvmin)/(ltpvmaxloc-ltpvminloc)

ltptslope=(ltptmax-ltptmin)/(ltptmaxloc-ltptminloc)

meteoPslope=(meteoPmax-meteoPmin)/(meteoPmaxloc-meteoPminloc)

meteoTslope=(meteoTmax-meteoTmin)/(meteoTmaxloc-meteoTminloc)

# agrupa las características
ltpv1=pd.concat([ltpvmax,ltpvmaxloc,ltpvmin,ltpvminloc,ltpvavg,ltpvvar,ltpvslope],keys=['maxltp','maxlocltp','minltp','minlocltp','avgltp','varltp','slopeltp'])
ltpv1=ltpv1.unstack(level=0).transpose()

ltpt1=pd.concat([ltptmax,ltptmaxloc,ltptmin,ltptminloc,ltptavg,ltptvar,ltptslope],keys=['maxltp','maxlocltp','minltp','minlocltp','avgltp','varltp','slopeltp'])
ltpt1=ltpt1.unstack(level=0).transpose()

meteoP_norm1=pd.concat([meteoPmax,meteoPmaxloc,meteoPmin,meteoPminloc,meteoPavg,meteoPvar,meteoPslope],keys=['maxmeteo','maxlocmeteo','minmeteo','minlocmeteo','avgmeteo','varmeteo','slopemeteo'])
meteoP_norm1=meteoP_norm1.unstack(level=0).transpose()

meteoT_norm1=pd.concat([meteoTmax,meteoTmaxloc,meteoTmin,meteoTminloc,meteoTavg,meteoTvar,meteoTslope],keys=['maxmeteo','maxlocmeteo','minmeteo','minlocmeteo','avgmeteo','varmeteo','slopemeteo'])
meteoT_norm1=meteoT_norm1.unstack(level=0).transpose()

# sustituye los vectores originales por los vectores de características
ltpv=ltpv1
ltpt=ltpt1
meteoP_norm=meteoP_norm1
meteoT_norm=meteoT_norm1
# # añade las características a sus vectores originales
# ltpv=pd.concat([ltpv,ltpv1],axis=0)
# ltpt=pd.concat([ltpt,ltpt1],axis=0)
# meteoP_norm=pd.concat([meteoP_norm,meteoP_norm1],axis=0)
# meteoT_norm=pd.concat([meteoT_norm,meteoT_norm1],axis=0)
...
res['error']=res['valdata']-res['estado']
res = res.transpose()


# guarda res en un archivo csv
res.to_csv('resClasPCALDAMeteoCaract'+year_train+year_data+sufix+'.csv')
