...
for n in n_list:
    ...
    # crea un dataframe con el el número de columnas de tdv_5_8_max_diff_sign y n filas con valor 1
    nappend = pd.DataFrame(np.ones((n,tdv_5_8_max_diff_sign.shape[1])),index=range(1,n+1),columns=tdv_5_8_max_diff_sign.columns)
    # convierte el índice de nappend a timedelta y lo resta al primer valor del índice de tdv_5_8_max_diff_sign
    nappend.index=pd.to_datetime(tdv_5_8_max_diff_sign.index[0])-pd.to_timedelta(nappend.index,unit="d")
    #convierte el índice a date
    nappend.index=nappend.index.date
    # añade nappend a tdv_5_8_max_diff_sign
    tdv_5_8_max_diff_sign=tdv_5_8_max_diff_sign.append(nappend)
    # ordena tdv_5_8_max_diff_sign por el índice
    tdv_5_8_max_diff_sign=tdv_5_8_max_diff_sign.sort_index()
