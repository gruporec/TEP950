...
class KriggingClassifier:
    ...
    def update_matrices(self, Xtrain, alpha):
        ...
        # Calcula la matriz G de tamaño 3*N x 2*N con cuatro matrices identidad de tamaño N y una columna de ceros. Todas las matrices identidad tienen signo negativo excepto la esquina superior derecha
        self.G = np.zeros([3*Xtrain.shape[1], 2*Xtrain.shape[1]])
        ...
        #crea una matriz auxiliar rectangular de tamaño 2Xtrain.shape[1]xXtrain.shape[1] con la primera mitad de 0 y la segunda una matriz identidad negativa
        aux = np.zeros([Xtrain.shape[1], 2*Xtrain.shape[1]])
        aux[:, Xtrain.shape[1]:2*Xtrain.shape[1]] = -np.eye(Xtrain.shape[1])

        #añade la matriz auxiliar a G en la posición inferior
        self.G[2*Xtrain.shape[1]:, :] = aux
        ...
        self.h = np.zeros([3*Xtrain.shape[1]])
    ...
    def apply(self,x):
        ...
        if T is None:
            ...
            #imprime t_i si alguno de sus valores es negativo
            if np.any(t_i < 0):
