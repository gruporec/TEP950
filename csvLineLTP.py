...
df = pd.read_csv("ignore/analisisPCALDA/resultadosPCALDAZIM.csv", index_col=[0,1,2,3],header=[0,1], skipinitialspace=True)
print(df)
#accuracies=df['total accuracy'].droplevel(3,'index').unstack(level=2).droplevel(0,'columns').sort_index('index',0)
accuracies=df['total accuracy'].droplevel(3,'index').unstack(level=2).droplevel(0,'columns').sort_index('index',0).droplevel(1,'index')
print(accuracies)
acc=accuracies
...
#fig=ax.get_figure()
plt.show()
fig.savefig('ignore/analisisPCALDA/ZIM Line.png')